...
public class TrainingAnnouncer {
    ...
    private void announce() {
        ...
        List<Training> upcomingTrainings = trainingRepository.findByDateTimeBetweenAndAnnounced(now, oneHourLater, false);
        ...
        for (String role : roleNames)
            api.getServers().forEach(server -> {
                server.getRolesByName(role).stream().findFirst().ifPresent(r -> rolesMention.append(r.getMentionTag()).append(" "));
            });

        for (Training training : upcomingTrainings) {
            api.getServers().forEach(server -> {
                server.getTextChannelById(server.getSystemChannel().get().getId()).ifPresent(channel -> {
                    channel.sendMessage("¡Atención!, recuerden la clase '" + training.getTrainingName() +
                            "' con el coach " + training.getCoach() +
                            " será a las " + training.getDateTime().toString() +
                            " no falten!" + rolesMention
                    );
                });
            });
        }
